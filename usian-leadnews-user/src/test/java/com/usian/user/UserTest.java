...
public void orcTest1(){
    ...
    if ("成功".equals(map1.get("message"))){
        ...
        Map map2 = JSONObject.parseObject(String.valueOf(response2),Map.class);
        if ("一致".equals(map2.get("message"))){     //   开始人像对比
            httpHeaders.add("apicode","0b92176a84d04dab8caa609c06892eec");
            httpHeaders.setContentType(MediaType.APPLICATION_JSON); // json
            Map map3=new HashMap();
            map3.put("idNumber",twomap.get("cardNum"));
            map3.put("userName",twomap.get("name"));
            HttpEntity<String> formEntry3 = new HttpEntity<>(JSON.toJSONString(map3), httpHeaders); // 封装请求参数
            ResponseEntity<String> response3 = restTemplate.postForEntity("https://api.yonyoucloud.com/apis/dst/matchIdentity/matchIdentity", formEntry3, String.class);// 发送一个post请求
            System.out.println(response3);
            Map map4 = JSONObject.parseObject(String.valueOf(response3),Map.class);
            String score = map4.get("score").toString();
            int fen = Integer.parseInt(score);
